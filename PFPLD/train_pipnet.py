...
from pfld.loss import PFLDLoss, MSELoss, SmoothL1, WingLoss, WingLoss_pose
...
def train(train_loader, plfd_backbone, criterion, optimizer,
    ...
    for img, target_map_gt, target_local_x_gt, target_local_y_gt, target_nb_x_gt, target_nb_y_gt, euler_angle_gt, landmark_gt, type_flag in train_loader:
        ...
        outputs_map, outputs_x, outputs_y, outputs_nb_x, outputs_nb_y, pose = plfd_backbone(img)
        ...
        total_loss = cls_loss_weight*loss_map + reg_loss_weight*loss_x + reg_loss_weight*loss_y + reg_loss_weight*loss_nb_x + reg_loss_weight*loss_nb_y
        # pose_loss = WingLoss_pose(pose, euler_angle_gt, type_flag)
        # total_loss += pose_loss

        batch_num += 1
        if batch_num % 50==0:
            print(
                'Epoch: %d,batch: %d, total_loss: %6.4f' %
                (epoch-1, batch_num, total_loss))
...
def validate(wlfw_val_dataloader, plfd_backbone, args):
    plfd_backbone.eval()
    # weight_file = '/data/cv/jiahe.lu/nniefacelib/PFPLD/models/checkpoint/pipnet/checkpoint_epoch_70.pth'
    ...
    with torch.no_grad():
        ...
        for img, target_map_gt, target_local_x_gt, target_local_y_gt, target_nb_x_gt, target_nb_y_gt, euler_angle_gt, landmark_gt, type_flag in wlfw_val_dataloader:
            ...
            lms_pred_x, lms_pred_y, lms_pred_nb_x, lms_pred_nb_y, outputs_cls, max_cls = forward_pip(plfd_backbone, img, transform, 112, 16, 10)
...
def main(args):
    ...
    # weight_file = '/data/cv/jiahe.lu/nniefacelib/PFPLD/models/checkpoint/pipnet/checkpoint_epoch_4.pth'
    # state_dict = torch.load(weight_file, map_location='cuda')
    # plfd_backbone.load_state_dict(state_dict)
...
def parse_args():
    ...
    parser.add_argument('--snapshot',default='./models/checkpoint/pipnet/',type=str,metavar='PATH')
