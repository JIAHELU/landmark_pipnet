...
def compute_loss_pip(outputs_map, outputs_local_x, outputs_local_y, outputs_nb_x, outputs_nb_y, labels_map, labels_local_x, labels_local_y, labels_nb_x, labels_nb_y,  criterion_cls, criterion_reg, num_nb, type_flag):
    landms_const = torch.tensor(-2).cuda()
    pos1 = type_flag == landms_const
    outputs_map = outputs_map[pos1, :, :, :]
    outputs_local_x = outputs_local_x[pos1, :, :, :]
    outputs_local_y = outputs_local_y[pos1, :, :, :]
    outputs_nb_x = outputs_nb_x[pos1, :, :, :]
    outputs_nb_y = outputs_nb_y[pos1, :, :, :]
    labels_map = labels_map[pos1, :, :, :]
    labels_local_x = labels_local_x[pos1, :, :, :]
    labels_local_y = labels_local_y[pos1, :, :, :]
    labels_nb_x = labels_nb_x[pos1, :, :, :]
    labels_nb_y = labels_nb_y[pos1, :, :, :]
...
def forward_pip(net, inputs, preprocess, input_size, net_stride, num_nb):
    net.eval()
    with torch.no_grad():
        outputs_cls, outputs_x, outputs_y, outputs_nb_x, outputs_nb_y, pose = net(inputs)
...
def forward_pip_landmark(net, inputs, preprocess, input_size, net_stride, num_nb):
    net.eval()
    with torch.no_grad():
        outputs_cls, outputs_x, outputs_y, outputs_nb_x, outputs_nb_y, landmark, pose = net(inputs)
        tmp_batch, tmp_channel, tmp_height, tmp_width = outputs_cls.size()
        assert tmp_batch == 1
    ...
    return tmp_x, tmp_y, tmp_nb_x, tmp_nb_y, outputs_cls, max_cls, landmark
