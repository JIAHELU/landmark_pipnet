...
class WLFWDatasets(data.Dataset):
    def __init__(self, file_list, transforms=None):
        ...
        self.meanface_indices, _, _, _ = get_meanface('/data/cv/jiahe.lu/nniefacelib/PFPLD/data/meanface.txt', 10)
    ...
    def __getitem__(self, index):
        ...
        target_map = np.zeros((self.num_lms, 7, 7))
        target_local_x = np.zeros((self.num_lms, 7, 7))
        target_local_y = np.zeros((self.num_lms, 7, 7))
        target_nb_x = np.zeros((self.num_nb*self.num_lms, 7, 7))
        target_nb_y = np.zeros((self.num_nb*self.num_lms, 7, 7))
        target_map, target_local_x, target_local_y, target_nb_x, target_nb_y = gen_target_pip(self.landmark, self.meanface_indices, target_map, target_local_x, target_local_y, target_nb_x, target_nb_y)
        ...
        # if self.type_flag == -2:
        #     os.makedirs(f'/data/cv/jiahe.lu/nniefacelib/PFPLD/test_related/grid_img/{index}')
        #     self.save_dir = f'/data/cv/jiahe.lu/nniefacelib/PFPLD/test_related/grid_img/{index}'
        #     self.save_grid_image(index)
        #     self.save_target_map_images(target_map, index)
        
        # print("Type flag:", self.type_flag)
        if self.transforms:
            self.img = self.transforms(self.img)
        return (self.img, target_map, target_local_x, target_local_y, target_nb_x, target_nb_y, self.euler_angle, self.landmark, self.type_flag)
    
    def save_grid_image(self, index):
        grid_position = self.landmark.reshape(98, 2)
        landmark = self.landmark.reshape(98, 2) * 112
        gt_file = os.path.join(self.save_dir, f'gt_{index}.txt')
        with open(gt_file, 'w') as f:
            for i in range(grid_position.shape[0]):
                x, y = int(grid_position[i, 0] * 7), int(grid_position[i, 1] * 7)
                f.write(f'Landmark {i}: ({x}, {y})\n')
        img_with_grid = np.array(self.img)
        img_with_grid = cv2.cvtColor(img_with_grid, cv2.COLOR_RGB2BGR)
        height, width, _ = img_with_grid.shape

        # 画出7x7网格
        for i in range(1, 7):
            # 画水平线
            start_point = (0, i * height // 7)
            end_point = (width, i * height // 7)
            img_with_grid = cv2.line(img_with_grid, start_point, end_point, (0, 255, 0), 1)

            # 画垂直线
            start_point = (i * width // 7, 0)
            end_point = (i * width // 7, height)
            img_with_grid = cv2.line(img_with_grid, start_point, end_point, (0, 255, 0), 1)
        
        for (x, y) in landmark.astype(np.int32):
            cv2.circle(img_with_grid, (x, y), 2, (0, 0, 255), -1)
                    
        cv2.imwrite(os.path.join(self.save_dir, f'grid_{index}.jpg'), img_with_grid)

    def save_target_map_images(self, target_map, index):
        # 打开或创建用于保存坐标的txt文件
        coords_file = os.path.join(self.save_dir, f'coords_{index}.txt')
        with open(coords_file, 'w') as f:
            for i in range(target_map.shape[0]):
                heatmap = target_map[i].numpy()
                max_y, max_x = np.unravel_index(np.argmax(heatmap), heatmap.shape)
                f.write(f'Landmark {i}: ({max_x}, {max_y})\n')
