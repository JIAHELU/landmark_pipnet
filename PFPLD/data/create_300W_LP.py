...
def get_args():
	...
	parser.add_argument("--db", type=str, default='IBUG',
	                    help="path to database")
	parser.add_argument("--root", type=str, default='/media/bill/Softmare-Document/ubuntu_yingpan/face_landmark/300W_LP/',
	                    help="path to database")
	parser.add_argument("--output", type=str, default='/media/bill/Softmare-Document/ubuntu_yingpan/face_landmark/300W_LP_output',
...
def main():
	...
	with open(os.path.join(args.output, args.db + '_label.txt'),'w') as fw:
		for i in tqdm(range(len(onlyfiles_jpg))):
			...
			lds_contents = sio.loadmat(os.path.join(args.root, 'landmarks', args.db, lds_name))
			...
			plot_pose_cube(img, yaw, pitch, roll, tdx=100, tdy=100,
						   size=50)
			...
			label = '{} {} 0 0 0 0 0 0 {}\n'.format(save_path, landmark_str, euler_angles_str) #98landmarks +6 attributes +pose +type_flag
			fw.write(label)
